...
> **注意（2026-09-25 起）**：本笔记调研的是「不渲染地读 DOCX 结构」这条路线，
> 该路线已被 [ADR-0008](../adr/0008-image-only-rasterize-first.md) 放弃——
> DOCX 改为先栅格化，于是**渲染这一步又回来了**。笔记里对本 ADR 仍然最有价值的部分
> 恰好是被否掉的那几个选项：LibreOffice 镜像体积、WASM 的 53 MB 传输与 1 GB 内存、
> 以及 headless 转换的 open bug——它们是 ADR-0008 的**已知成本清单**，
> 而不是可以忽略的旧结论。
