...
> **注意（2026-09-25 起）**：本笔记调研的是「读文档自带的文字层」这条路线，
> 该路线已被 [ADR-0008](../adr/0008-image-only-rasterize-first.md) 放弃——
> 资源一律先栅格化，不再读字。笔记中的事实性结论（PyMuPDF 能力、`sort=True` 的局限、
> 判页启发式等）仍然成立且可引用，但**不要再据此设计解析管线**。
> 仍然有用的一条：多栏阅读顺序即使有文字层也要上版面模型——这佐证了
> 「版面理解必须靠视觉模型」这个前提在纯位图路线下只会更成立。
...
> **2026-09-25 状态复核**（依 [ADR-0008](../adr/0008-image-only-rasterize-first.md)）。
> ADR-0008 放弃文字层路线后，下列条目**作废**——它们验证的是「怎么读文字层」，
> 而这条路径不再存在：**2、3、5、9**（多栏排序、自研分栏 vs 版面模型、DOCX 归属信息、
> 文字层是否可信）。注意它们的**上位问题没有消失**：「阅读顺序怎么办」在纯位图下
> 改由视觉模型回答，因此仍是必须验证的，只是换成了下面新列的第 11 条。
> 同时 **7 的优先级从「基线」升为「关键路径」**：栅格化现在是全管线的第一步，
> 它的耗时与内存直接决定整批资料的墙钟时间。
> **8 依然是最高优先**：它决定 OCR 是不是必须项——纯位图路线下这个问题更硬。

1. **`find_tables(use_layout=True)` 到底依赖什么**——是否引入模型/额外依赖，与 `use_layout=False` 的结果差异。官方文档口径不一致，必须实测。
   （对应未确认项：4.1 末）**〔已作废：不再读文字层〕**
2. ~~**PyMuPDF `sort=True` 在本校真实资料上的失败率**~~ **〔已作废〕**
3. ~~**自研分栏 vs `pymupdf-layout` 的准确率/速度对比**~~ **〔已作废〕**
4. **LibreOffice DOCX→PDF 的保真度实测**——**〔优先级上升〕**：ADR-0008 后这是 DOCX 的必经之路。
   重点测：中文字体缺失时的替换行为与分页漂移、文本框/浮动图片位置、表格跨页。
   官方承认字体替换存在、Bugzilla 有分页漂移的已确认案例，但**没说影响多大**。
5. ~~**DOCX 直解能给出的"归属信息"够不够用**~~ **〔已作废〕**
6. ~~**许可合规确认**~~ **〔已解决：2026-09-25 用户确认接受 AGPL-3.0〕**
7. **渲染成本基线**——**〔关键路径〕**。用 `get_pixmap(dpi=150/200)` 测单页渲染时间与峰值内存
   （官方只给了"multiprocessing 约快一倍"与 OCR 约慢 1000× 两个锚点，没有单页绝对数字），
   据以估算整批资料的墙钟时间，**并据此选栅格化 DPI**。
8. **扫描件的实际占比**——**〔最高优先〕**。但判据要改：原为「`get_text()` 返回空/近乎空的
   页比例」，在纯位图路线下**这个统计失去意义**（一切都当图片看）。
   改为统计**输入资源的格式构成**：图片 : PDF : DOCX 的比例，以此决定 DOCX
   栅格化（需要 LibreOffice）值不值得在首版做。
9. ~~**是否存在官方警告的两种"文字层不可信"情况**~~ **〔已作废〕**
10. **表格提取可用性**——`find_tables` 在无框线（纯 text 策略）表格上的表现，以及 `use_layout` 开关的影响。**〔已作废：不再读文字层〕**
11. **纯位图下的版面理解精度**（新）——在栅格图上跑版面检测 + OCR，
    验证「切分粒度够不够细」「阅读顺序判得对不对」「字太小的块放大后能否辨认」。
    这一条取代作废的 2/3/5，是 ADR-0008 补偿方案（切碎到放大够用）的直接检验。
